...
````JavaScript
let timepicker = $find("<%= RadTimePicker1.ClientID %>");
````
...
function incrementTime(picker) {
    let date = picker.get_selectedDate();
...
````JavaScript
function getTime() {
    let picker = $find("<%=RadDateTimePicker1.ClientID%>");
    let view = picker.get_timeView();
...
````JavaScript
function disable() {
    let picker = $find("<%= RadTimePicker1.ClientID %>");
