...
The __OnClientLoad__ client-side event occurs when the tab strip is first loaded and fully initialized on the client.
...
>note The technique illustrated here can be used when the tab strip is nested within another control, but the __RadTabStrip__ must be rendered only once. For example, you can use this approach if __RadTabStrip__ is nested in a tool bar template button. However, if __RadTabStrip__ is nested in a grid with more than one row, the method fails since there will be multiple instances of the rendered tab strip.
>
