...
You should define at least the **DataTextField** property of the tag cloud and, optionally, the rest of the properties that end with **Field** (you can check them in the [server-side API article]({%slug tagcloud/server-side-programming/tagcloud-object%}) of the control). These properties provide the field names from which the tag cloud takes thedata for each item, as shown in **Example 1** and **Example 2**.
