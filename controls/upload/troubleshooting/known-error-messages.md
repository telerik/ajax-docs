...
page_title: Known Error Messages | UI for ASP.NET AJAX Documentation
...
When you try to use the __RadUpload__ or a standard __HtmlInputFile__ control to upload a large file, the file may not be uploaded.

## CAUSE

This problem occurs because the default value for the __maxRequestLength__ parameter in the section of the Machine.config file is 4096 (4 megabytes). As a result, files that are larger than this value are not uploaded by default.
