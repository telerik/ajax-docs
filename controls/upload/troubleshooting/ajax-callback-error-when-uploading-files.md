...
page_title: Ajax Callback Error When Uploading Files | UI for ASP.NET AJAX Documentation
...
1. The __Telerik.RadUploadProgressHandler.ashx__ is not registered. In this case, the error message title is followed by "__Source url was not found__".

1. A server-side error has occurred. The error message title is followed by "__Source url returned invalid content__". This indicates that the server-side progress data calculation raised an unhandled exception.
