...
* Add an **input** element of type "**check box"** to add a boolean field to theuploaded file.

1. For each **input** element you add, assign its **id** and **name**attributes to a value that you obtain using the **RadUpload.getID(name)** method. The id and name must begenerated by the **RadUpload** control so that it can locate the values server-side.
...
>caution The following example adds **<LI>** elements to the UL list of the RadUpload. Note that **MaxFileInputsCount** property counts the number of **<LI>** elements so you need touse it with attention. Check the[online example](http://demos.telerik.com/aspnet/prometheus/Upload/Examples/AdditionalFields/DefaultCS.aspx)for another approach.
>
