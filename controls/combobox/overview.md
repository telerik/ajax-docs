...
__Telerik RadComboBox for ASP.NET AJAX__ is a flexible drop-down control for ASP.NET applications. __RadComboBox__ introduces a number of advanced features, including:
...
* __Overlay support__. RadComboBox can display its drop-down list over IE windowed objects suchHTML select boxes, Flash movies, and so on. This built-in feature does not need to be set explicitly through a property.

* __Screen boundary detection__.The drop-down expands to the opposite direction when necessary to prevent screen boundaries from being crossed.
