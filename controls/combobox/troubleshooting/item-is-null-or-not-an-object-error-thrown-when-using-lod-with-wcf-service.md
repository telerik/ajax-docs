...
page_title: Item is null or not an object error Thrown When Using LoD with WCF service | UI for ASP.NET AJAX Documentation
...
The __handlerPath__ is relative to the root of the application and if the service is placed inside a specific folder, it should be also included in the path. The __matchExact__ attribute determines whether the rule will be forced for the specified path only or globally for the entire web site/web application project

# See Also

 * [Loading Items from WCF Service]({%slug combobox/load-on-demand/loading-items-from-wcf-service%})

 * [Loading Items from WCF Service Demo](http://demos.telerik.com/aspnet-ajax/combobox/examples/loadondemand/wcf/defaultcs.aspx)
