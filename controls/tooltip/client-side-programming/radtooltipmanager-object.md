...
| Name | Description |
| ------ | ------ |
| **createToolTip** |creates a new tooltip on the client which is associated with a corresponding target element
...
|
| **getToolTipByElement** |gets the tooltip associated with a corresponding target element if such a tooltip exists
...
| **set_autoTooltipify** |Sets a boolean value that indicates whether automatic RadToolTip creation for DOM elements with`title`or`alt`attributes is enabled.This can be used to create tooltips only for a specific are on demand, without having the automatic feature enabled for the entire page:
