...
page_title: CustomCodeField | UI for ASP.NET AJAX Documentation
description: CustomCodeField
slug: wordsprocessing/concepts/fields/customcodefield
...
__Fields__ in __RadFlowDocument__ consist of code fragment and result fragment as explained in the [Fields]({%slug wordsprocessing/concepts/fields/fields%}) article. Some fields have direct representation in the document model – for example [Hyperlinks]({%slug wordsprocessing/concepts/hyperlinks%}). For all other fields you can use the __CustomCodeField__ class – however you will need some knowledge of how to correctly form the code of the field.
...
* *switches*: Additional properties of the field. More information on the topic can be found in the [Fields]({%slug wordsprocessing/concepts/fields/fields%}) article.

## Inserting

The suggested approach for inserting code fields is by using [RadFlowDocumentEditor]({%slug wordsprocessing/editing/radflowdocumenteditor%}). The __InsertField()__ method accepts code as a first argument and the result as a second argument.
