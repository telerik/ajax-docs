...
page_title: Formats and Conversion | UI for ASP.NET AJAX Documentation
...
RadWordProcessing does not need any external dependencies in order to convert documents from/to the supported formats. The document model is independant from UI and can be used on the server side as well as on the client.
...
>note *All fields are supported through custom code fields. More information is available[here]({%slug wordsprocessing/concepts/fields/customcodefield%}).
>


>note **Floating images are treated as inline images when importing/exporting HTML.
>
