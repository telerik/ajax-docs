...
page_title: Understanding the Skin CSS File | RadColorPicker for ASP.NET AJAX Documentation
...
Each style maps to a "class" attribute in an HTML tag. For example, consider the HTML rendering of a **RadColorPicker** control:
...
The **RadColorPicker** control is rendered as a Table element with the class "radcolorpicker RadColorPicker_Default". The color boxes are rendered as a series of list item "`<li>`" tags that each contain a link "`<a>`" tag.
