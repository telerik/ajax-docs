...
The control also exposes the **AutoPostBack** property which indicates whetheran automatic postback will be triggered once a tile is selected. It defaults to false.
...
A given **tile** selection status can be toggled through its **Selected** property. It can be setin the markup, the code-behind or though JavaScript through the set_selected(isSelected) method each tile object exposes.
