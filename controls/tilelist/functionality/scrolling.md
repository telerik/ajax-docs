...
* Accelerated - the displayed scrollbar is custom styled and accelerated. It is recommended that it is used only for touch devices, yet it can bringtouch-scrolling experience to desktop users as well - the user can hold down the mouse button and scroll as if it werehis/her finger on the screen.

The ScrollingMode property can be changed on the server (or in the markup) and on the client with JavaScript through the*set_scrollingMode()* method. It takes a member of the	*Telerik.Web.UI.TileList.TileListScrollingMode* enum which has the same nameboth on the server and on the client.
