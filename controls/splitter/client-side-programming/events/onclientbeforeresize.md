---
title: OnClientBeforeResize
page_title: OnClientBeforeResize | UI for ASP.NET AJAX Documentation
description: OnClientBeforeResize
slug: splitter/client-side-programming/events/onclientbeforeresize
tags: onclientbeforeresize
published: False
position: 3
---
...
>caution  __OnClientBeforeResize__ Client-Side event has been deprecated. Please, use the[OnClientResizing]({%slug splitter/client-side-programming/events/onclientresizing%})instead.
>


The __OnClientBeforeResize__ client-side event handler is called when a control is about to be resized. The resize can occur because of a user action (such as dragging a split bar or resizing the window that contains a splitter), or as the result of a a call to a client-side method.

>note The __OnClientBeforeResize__ event is supported by __RadSplitter__ , __RadPane__ , and __RadSlidingPane__ .
>
...
The following example uses the __OnClientBeforeResize__ event to prevent resizes that decrease the size of a pane:
...
````JavaScript
	...
	<telerik:RadSplitter runat="server" id="RadSplitter1" >
	 ...
	 <telerik:RadPane runat="server"
	     id="RadPane2"
	     Height="20px"
	     OnClientBeforeResize="preventDecrease">
...
````ASPNET
	...
	<telerik:RadSplitter
	  ...
	  Width="90%"
	  Height="90%"
	  OnClientBeforeResize="testHandler">
	 <telerik:RadPane runat="server" id="RadPane1" Height="80px"   >
	 </telerik:RadPane>
	 <telerik:RadSplitBar runat="server" id="RadSplitBar1" CollapseMode="Both"  />
