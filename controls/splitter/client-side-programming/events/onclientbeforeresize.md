...
page_title: OnClientBeforeResize | RadSplitter for ASP.NET AJAX Documentation
...
>caution  **OnClientBeforeResize** Client-Side event has been deprecated. Please, use the[OnClientResizing]({%slug splitter/client-side-programming/events/onclientresizing%})instead.
>


The **OnClientBeforeResize** client-side event handler is called when a control is about to be resized. The resize can occur because of a user action (such as dragging a split bar or resizing the window that contains a splitter), or as the result of a a call to a client-side method.

>note The **OnClientBeforeResize** event is supported by **RadSplitter** , **RadPane** , and **RadSlidingPane** .
>
...
The following example uses the **OnClientBeforeResize** event to prevent resizes that decrease the size of a pane:
