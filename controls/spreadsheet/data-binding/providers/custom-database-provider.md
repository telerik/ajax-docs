...
Public Class SpreadsheetDataBaseProvider Inherits SpreadsheetProviderBase
...
3. JavaScript keeps the dates as interegers and in the save method, the dates would would be received as integers, not DateTime objects. To convert the integer to a date on the server, the following approach can be used: 
...
4. Provide the RadSpreadsheet control with the newly created custom provider at the Page_Load in the following manner: 
...
>note You can investigate the entire setup and test the behavior of the **RadSpreadsheet** with **Custom Database Provider** in the [Custom Database Provider for RadSpreadsheet]({%slug spreadsheet-database-provider%}) article.
