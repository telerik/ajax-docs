...
````VB
	' Create a FlowDocument
	Dim flowDocument As New docx.RadFlowDocument()
	
	' Create a Document Section
	Dim section As docx.Section = flowDocument.Sections.AddSection()
	
	' Insert a Table into the Document Section
	Dim table As docx.Table = section.Blocks.AddTable()
	
	' Create Cell Border Style
	Dim tableCellBorders As New docx.Styles.TableCellBorders(
	    New docx.Styles.Border(docx.Styles.BorderStyle.Single)
	)
	
	' Loop for creating 5 rows and 4 cells per row
	For rowIndex As Integer = 0 To 4
	    ' Insert a row into the table
	    Dim row As docx.TableRow = table.Rows.AddTableRow()
	
	    For colIndex As Integer = 0 To 3
	        Dim cell As docx.TableCell = row.Cells.AddTableCell()
	...
	' First cell in the first row
	Dim selectedCell As docx.TableCell = table.Rows(0).Cells(0)
	
	' Add a paragraph to the cell
	Dim paragraph As docx.Paragraph = selectedCell.Blocks.AddParagraph()
	
	' Set paragraph value
	paragraph.Inlines.AddRun("First Cell First Row")
	
	' Fourth cell in the fifth row
	selectedCell = table.Rows(4).Cells(3)
...
1. Using/Imports statement
	...
	````
    ````VB
	    Imports Telerik.Documents.Spreadsheet.FormatProviders
	    Imports Telerik.Documents.Spreadsheet.FormatProviders.OpenXml.Xlsx
