...
````
````VB
Public MustInherit Class DbSchedulerProviderBase Inherits SchedulerProviderBase
	...
	...
	End Function
	Protected Virtual Function CreateParameter( ByVal name As String, ByVal value As Object) As DbParameter
	...
	End Function
End Class
````
...
````
````VB
Public MustInherit Class MyDbSchedulerProvider Inherits DbSchedulerProviderBase
	' ...
End Class
````
	...
	Using conn As DbConnection = OpenConnection()
		...
		Using reader As DbDataReader = cmd.ExecuteReader()
			While reader.Read()
				...
				apt.RecurrenceRule = Convert.ToString(reader("RecurrenceRule"))
				apt.RecurrenceParentID = IIf(reader("RecurrenceParentId") = DBNull.Value, Nothing, reader("RecurrenceParentId"))
...
Public Overloads Overrides Sub Insert(ByVal owner As RadScheduler, ByVal appointmentToInsert As Appointment)
	...
	Using conn As DbConnection = OpenConnection()
		Using tran As DbTransaction = conn.BeginTransaction()
			...
			cmd.CommandText = "INSERT INTO [DbProvider_Classes] " & _
                    "([Subject], [Start], [End], [TeacherID], " & _
                    "[RecurrenceRule], [RecurrenceParentID]) " & _
                    "VALUES (@Subject, @Start, @End, @TeacherID, " & _
                    "@RecurrenceRule, @RecurrenceParentID)"
