---
title: Telerik.Web.UI.GridCheckBoxColumn
page_title: Telerik.Web.UI.GridCheckBoxColumn
description: Telerik.Web.UI.GridCheckBoxColumn
---

# Telerik.Web.UI.GridCheckBoxColumn

Displays a CheckBoxcontrol for each item in the column. This allows you
            to edit for example Booleanfield(s) from data table(s).

#### Remarks
This column is editable (implements the IGridEditableColumninterface) and provides by default GridBoolColumnEditor, used for
            editing the text in each item. You can persist the checked state of a checkbox, if you
            use it within GridTemplateColumn(gridPersistCheckBoxStateInGridTemplateColumnOnRebind.html see here).
...
###  StringTrueValue `String`

Use this property when binding to string column that contains boolean values written as string.
            For example if the values in your column are "Yes" and "No", set StringTrueValue="Yes" and StringFalseValue="No"

###  StringFalseValue `String`
...
The ToolTip that will be applied to every item  control.
