...
This column renderes a button of the specified ButtonType in each corresponding
                    cell of the items of type Telerik.Web.UI.GridDataItem and
                    Telerik.Web.UI.GridEditFormItem . You can use this buttons to fire command
                    events that can be handeled in E:Telerik.Web.UI.RadGrid.ItemCommand event
                    handler. This, in combination with the
                    grdCommandReference.html event
                    bubbling mechanismin Telerik RadGrid, allows you to create a
                    column of custom button controls, such as Add,
                    Remove, Selector Editbuttons.
                The available buttons types are:
                PushButton,LinkButtonand ImageButton.
                Telerik RadGrid comes with two types of button columns:bullet Select- when a button in this column is pressed, it
                    will select the whole row. The Selectcolumn below uses a
                    PushButton.Remove selection- when a button in this column is
                    pressed, it will delete the row. The Remove selectioncolumn
