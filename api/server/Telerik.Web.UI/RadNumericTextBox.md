---
title: Telerik.Web.UI.RadNumericTextBox
page_title: Telerik.Web.UI.RadNumericTextBox
description: Telerik.Web.UI.RadNumericTextBox
---
...
RadNumericTextBox restricts user input to numeric values. It shares the common properties
            ...
            You can find information on the server API of RadNumericTextBox at https://docs.telerik.com/devtools/aspnet-ajax/controls/numerictextbox/server-side-programming/properties .
...
###  ButtonDownContainer `HtmlGenericControl`
...
###  ButtonUpContainer `HtmlGenericControl`
...
That property is designed to be used when this control is
                    embedded into grid or other data-bound control.Default value is set to the Double.
...
###  DbValueFactor `Nullable`1`

Gets or sets a value indicating the Multiplication factor used in DbValue property.

#### Remarks
Use this property when you want to multiply value bound from the database by some factor,
             and then to devide by the same factor when saving it back to the database.
...
###  IncrementSettings `InputIncrementSettings`

Gets the settings associated with 
            increment settings determining the behavior when using the mouse wheel and keyboard arrow keys.
...
Use this property to provide a custom style for the negative state of
                ...
                foreground color, background color, font, and alignment within the RadInput.
                Providing a different style enhances the appearance of the RadInput
                control.Negative style properties in the RadInput control are inherited from one
                style property to another through a hierarchy. For example, if you specify a red
                font for the EnabledStyle property, all other style properties in the RadInput
                control will also have a red font. This allows you to provide a common appearance
                for the control by setting a single style property. You can override the inherited
                ...
                property.To specify a custom style, place the <NegativeStyle> tags between the
                opening and closing tags of the RadInput control. You can then list the style
                attributes within the opening <NegativeStyle> tag.

###  NumberFormat `NumberFormatSettings`

Gets the numeric format of the RadNumericTextBox control.
...
###  SpinDownCssClass `String`

Gets or sets CSS class for the spin down button.

###  SpinUpCssClass `String`

Gets or sets CSS class for the spin up button.
