...
slug: moss/sharepoint-2007-(moss)/radeditor-for-moss/configuration/set-properties-via-config-file
...
The same approach is used when setting __FlashPaths__, __DocumentsPaths__, __MediaPaths__ etc. properties. If these properties are not set, by default RadEditor will load all local libraries (local to the current site). For more information, see [Image Manager]({%slug moss/sharepoint-2007-(moss)/radeditor-for-moss/dialogs/image-manager%})
